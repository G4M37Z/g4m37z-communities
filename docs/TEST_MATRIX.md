...
| Official platform logos on profile + platform-links form | L3 (prod build, real clicks) | settings UI → Steam link saved → profile renders the Steam glyph in official brand color (dark-theme lifted hex via `--platform-brand`, computed style verified) | 2026-09-21 |
| Live message delivery in an open thread (realtime + safety nets) | L3 (prod build, cross-user insert) | SQL-inserted row from autotest2 appeared in autotest's open thread; `ThreadLive` re-subscribes on channel loss + visibility refresh + 30s fallback | 2026-09-21 |
| Seeded game covers (all 8 games) | L3 (live DB + rendered page) + L2 | 051 upgrades to Steam `library_600x900` / IGDB `t_720p` (all 8 verified `200`); `/discover` renders 8/8 at 300×450 in 2:3 cards (screenshot pass); `tests/game-cover-url.test.ts` 5/5 | 2026-09-21 |
