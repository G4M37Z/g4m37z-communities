...
> **PRE-LAUNCH (do not ship until done):** re-enable email confirmation before
> any public launch. Project `zpirpbivhkscixbokpbt` currently has "Confirm
> email" ENABLED; the approved disable-for-testing plan lives entirely in the
> Supabase dashboard (`authentication → providers → email`) — a DASHBOARD-ONLY
> toggle, since ox-auth settings live outside Postgres (no `auth.config`
> table) and cannot be changed via SQL. If it is disabled for testing, turn it
> BACK ON before launch. Details: `docs/ERRORS.md`.
