...
export interface RealtimeSubscriptionOptions { schema: string; table: string; event: RealtimeEvent; filter?: string; callback: (payload: RealtimePayload) => void; }
...
export function subscribe(supabase: SupabaseClient, opts: RealtimeSubscriptionOptions) {
  ...
  const channel = supabase.channel(channelName).on("postgres_changes", { event, schema, table, ...(filter ? { filter } : {}) }, (payload: any) => {
    callback({ eventType: payload.eventType as RealtimeEvent, new: payload.new, old: payload.old });
  }).subscribe();
